...
| G4. Queue Penilaian Esai | Selesai — terverifikasi | Route `guru/penilaian-esai/page.tsx:15-34,74-150`, service query `NEEDS_REVIEW`, pagination, dan deep link koreksi nyata; Week 11 lulus dan scope kelas dibuktikan. | ✅ 26 Sep 2026: deskripsi metric memakai `formatUiLabel("NEEDS_REVIEW")`; tidak ada lagi string enum mentah. |
...
| G6. Lifecycle sesi edit/cancel | Selesai — terverifikasi | `session-workspace.tsx:50-132` memiliki create/PATCH/DELETE, alasan override Admin, ownership service, dan `ConfirmDialog` cancel. | ✅ 26 Sep 2026: `tests/e2e/guru-workspace.spec.ts` menguji create → edit → cancel end-to-end. |
...
| G12. Publish langsung tanpa konfirmasi | Selesai sebagian | ✅ 26 Sep 2026: `material-status-actions.tsx`, `exam-status-actions.tsx`, dan `hero-carousel-actions.tsx` memakai `useConfirmDialog`. | Sisa `window.confirm` hanya tombol bersihkan di pemutar publik (`public-quiz-runner.tsx:432`); alur publish materi/ujian belum punya dialog pratinjau audience. |
| G13. Navigation menampilkan fitur yang production-default off | Selesai — terverifikasi | ✅ 26 Sep 2026: `getNavigationForRole` memfilter item lewat `requiredFeatures`; halaman kelas `[kelasId]` kini men-gate "Susun Modul" (`learningModulesEnabled`) dan "Progres Aktivitas" (`activityCompletionEnabled` + `learningModulesEnabled`); `tests/e2e/production-navigation.spec.ts` memverifikasi nav terlihat tidak 404 dan sub-fitur kelas tersembunyi saat semua flag mati. | Semua flag produksi tetap perlu diaktifkan lewat env saat deploy (lihat `docs/REDEPLOY_CHECKLIST.md`). |
...
| W10. Gradebook menampilkan provisional score | Selesai — terverifikasi | ✅ 26 Sep 2026: `getStudentGradebook`/`getWaliGradebook` memakai `exposeProvisionalScores:false` (`gradebook-service.ts:353-359,378,384`), `calculatedScore`/`letterGrade`/skor kategori di-null-kan, dan `gradebook-viewer.tsx:11-17` hanya menampilkan `publishedScore`. | `tests/run-week7-integration.mjs:106-123` memverifikasi draft/provisional tersembunyi (assertion eksplisit). |
...
| T8. Raw enum/internal jargon | Selesai sebagian | `ui-labels.ts` memusatkan label + `humanizeEnumLabel` sebagai fallback; ✅ 26 Sep 2026: `admin/audit/page.tsx:93,102` dan `guru/penilaian-esai/page.tsx:48` tidak lagi mencetak token mentah. | Fallback title-case masih berbahasa Inggris untuk aksi audit yang belum punya label eksplisit. |
...
| Gradebook Wali/Siswa hanya menampilkan nilai published | ✅ 26 Sep 2026: `gradebook-service.ts` memakai `exposeProvisionalScores:false` untuk Siswa/Wali dan `gradebook-viewer.tsx:11-17` hanya merender `publishedScore`; `tests/run-week7-integration.mjs:106-123` memverifikasi draft/provisional tersembunyi. | Selesai — terverifikasi |
| Navigation dan route guard feature baru konsisten | ✅ 26 Sep 2026: `getNavigationForRole` memfilter lewat `requiredFeatures` dan halaman kelas `[kelasId]` men-gate tautan sub-fitur; `tests/e2e/production-navigation.spec.ts` memverifikasi nav terlihat tidak 404 saat semua flag mati. | Selesai — terverifikasi |
...
| R2 | Navigation menampilkan route yang dimatikan production | ✅ 26 Sep 2026 diperbaiki: `getNavigationForRole` memfilter lewat `requiredFeatures`, dan halaman kelas Guru men-gate "Susun Modul"/"Progres Aktivitas"; `tests/e2e/production-navigation.spec.ts` memverifikasi. | Selesai — menu tidak lagi mengarah ke 404 saat flag produksi mati. |
...
| R5 | Gradebook implementation dapat mengekspos calculated score provisional | ✅ 26 Sep 2026 diverifikasi tertutup: `exposeProvisionalScores:false` untuk Siswa/Wali (`gradebook-service.ts`) dan `gradebook-viewer.tsx` hanya merender `publishedScore`; `tests/run-week7-integration.mjs:106-123` menguji draft-hidden. | Selesai — privacy gradebook Wali/Siswa teruji. |
| R6 | Handler form memanggil `event.currentTarget.reset()` setelah `await` | React men-null-kan `currentTarget` setelah event selesai, sehingga pemanggilan melempar `Cannot read properties of null (reading 'reset')` dan `router.refresh()` tidak pernah jalan (contoh: sesi baru tidak muncul tanpa reload). Ditemukan oleh `tests/e2e/guru-workspace.spec.ts`. | ✅ 26 Sep 2026: `session-workspace.tsx` & `hasil-ujian-form.tsx` diperbaiki (capture elemen sebelum `await`); pola sama masih ada di ~11 form lain (lihat `docs/KNOWN_LIMITATIONS.md`), belum semuanya diperbaiki. |
