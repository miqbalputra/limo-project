...
export function StudentAccountForm({ studentId, studentName, defaultIdentifier, account }: { studentId: string; studentName: string; defaultIdentifier: string; account: Account }) {
  ...
  async function request(path: string, options: Omit<RequestInit, "body"> & { body?: unknown }) {
    const response = await requestJson<{ activationUrl?: string }>(path, { ...options, fallbackMessage: "Aksi akun siswa gagal" });
    if (response.data.activationUrl) setActivationUrl(response.data.activationUrl);
    router.refresh();
  }
  ...
  return (
    <>
      ...
      {error && !pendingStatus ? <p role="alert" className="mt-4 tailadmin-alert-error">{error}</p> : null}
      {!account ? (
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="mt-1"><StatusBadge status={account.status} compact /></div>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {account.status === "INACTIVE" ? <button type="button" onClick={() => setPendingStatus("ACTIVE")} disabled={isSubmitting} className="tailadmin-button-primary px-3 py-2 text-theme-xs">Aktifkan</button> : null}
            {account.status === "ACTIVE" ? <button type="button" onClick={() => setPendingStatus("INACTIVE")} disabled={isSubmitting} className="tailadmin-button-outline px-3 py-2 text-theme-xs">Nonaktifkan</button> : null}
          </div>
        </div>
      )}
      ...
      <ConfirmDialog open={pendingStatus !== null} title={pendingStatus === "INACTIVE" ? "Nonaktifkan akun siswa?" : "Aktifkan akun siswa?"} description={pendingStatus === "INACTIVE" ? "Akses portal siswa akan dinonaktifkan dan seluruh sesi aktifnya dicabut." : "Akses portal siswa akan diaktifkan kembali."} confirmLabel={pendingStatus === "INACTIVE" ? "Ya, nonaktifkan" : "Ya, aktifkan"} variant={pendingStatus === "INACTIVE" ? "destructive" : "status"} isBusy={isSubmitting} error={error} onClose={() => { if (!isSubmitting) setPendingStatus(null); }} onConfirm={() => { if (pendingStatus) void updateStatus(pendingStatus); }} />
    </>
  );
}
