...
export async function getStudentDashboard(actor: Actor) {
  ...
  const [sessions, materials, exams, assignments, scores, progress, attendance] = await Promise.all([
    ...
    prisma.ujian.findMany({
      ...
      select: {
        ...
        attempts: { where: { siswaId: account.siswa.id, siswaAccountId: { not: null } }, orderBy: { createdAt: "desc" }, take: 1, select: { id: true, status: true, expiresAt: true } },
      },
    }),
    ...
    prisma.hasilUjian.findMany({
      where: { siswaId: account.siswa.id, status: { in: ["FINAL", "CORRECTED"] }, OR: [{ ujian: { showResultToSiswa: true } }, { releasedAt: { not: null } }] },
  ...
  return {
    ...
    exams: exams.map((item) => ({ ...item, latestAttempt: item.attempts[0] ?? null, attempts: undefined })),
    scores,
    progress,
