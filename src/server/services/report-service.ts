    ...
    select: {
      siswa: {
        select: {
          ...
          name: true,
          nomorInduk: true,
          presensi: { where: { sesiKelas: { kelasId } }, select: { status: true } },
          progresBelajar: { where: { sesiKelas: { kelasId } }, orderBy: { createdAt: "desc" }, take: 5, select: { understandingScore: true } },
          hasilUjian: { where: { status: { in: ["FINAL", "CORRECTED"] }, ujian: { kelasId } }, select: { totalScore: true } },
        },
      },
    },
  });
