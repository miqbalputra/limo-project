...
async function loadCandidates(now: Date) {
  ...
  for (const participant of remedials) {
    ...
    const accountEmail = enrollment.siswa.siswaAccount?.user.email;
    if (!accountEmail) continue;
    candidates.push({ recipient: accountEmail, recipientRole: "SISWA", sourceType: "RemedialAssignment", sourceId: participant.remedial.id, title: participant.remedial.title, className: participant.remedial.kelas.name, childName: null, dueAt, href: `/siswa/tugas/${participant.remedial.sourceId}?remedialId=${participant.id}`, siswaId: participant.studentId });
    for (const email of relationsByStudent.get(participant.studentId) || []) candidates.push({ recipient: email, recipientRole: "WALI", sourceType: "RemedialAssignment", sourceId: participant.remedial.id, title: participant.remedial.title, className: participant.remedial.kelas.name, childName: enrollment.siswa.name, dueAt, href: `/wali/progres/${participant.studentId}/remedial`, siswaId: participant.studentId });
  }
...
async function createReminderNotification(input: { recipient: string; role: string; window: ReminderWindow; dateKey: string; sourceKey: string; subject: string; body: string; metadata: Record<string, string | null> }) {
  ...
  const dedupeKey = createHash("sha256").update(rawKey).digest("hex");
  if (await prisma.notifikasi.findUnique({ where: { dedupeKey }, select: { id: true } })) return false;
  try {
    await prisma.notifikasi.create({ data: { channel: "in_app", template: "deadline-reminder", recipient: input.recipient, subject: input.subject, body: input.body, dedupeKey, metadata: input.metadata } });
    return true;
  } catch (error) {
