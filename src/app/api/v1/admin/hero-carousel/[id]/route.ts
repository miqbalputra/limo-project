...
export async function PATCH(request: Request, context: Context) {
  ...
  try {
    ...
    const input = Object.fromEntries(["sortOrder", "isActive", "eyebrow", "title", "subtitle", "description", "textPosition", "textTheme", "ctaLabel", "ctaHref", "cta2Label", "cta2Href", "altText"].filter((key) => form.has(key)).map((key) => [key, form.get(key) ?? ""]));
