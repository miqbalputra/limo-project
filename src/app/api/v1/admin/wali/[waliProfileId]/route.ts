...
import { archiveWali, getWali, updateWali } from "@/server/services/people-service";
...
export async function DELETE(request: Request, context: { params: Promise<{ waliProfileId: string }> }) {
  const requestId = getRequestId(request.headers);
  try {
    assertSameOrigin(request.headers, getEnv().APP_URL);
    const actor = await requireActor();
    requireRole(actor, ["ADMIN"]);
    const { waliProfileId } = await context.params;
    return apiOk(await archiveWali(actor, waliProfileId), { requestId });
  } catch (error) {
    return apiError(error, { requestId });
  }
}
