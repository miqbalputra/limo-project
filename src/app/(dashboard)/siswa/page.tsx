  ...
  return (
    <main className="space-y-6">
      <DashboardHero
        ...
        aside={<div className="rounded-2xl bg-limo-blue-500 px-5 py-4 text-white shadow-theme-lg"><p className="text-theme-xs text-white/70">Program</p><p className="mt-1 text-lg font-semibold">{dashboard.profile.program.name}</p><p className="mt-1 text-theme-xs text-white/80">{dashboard.profile.nomorInduk}</p></div>}
      />
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Materi Baru" value={dashboard.materials.length} description="Materi terbaru yang diterbitkan" icon="materials" tone="warning" />
      ...
      <section>
        <SectionHeader title="Kegiatan Terdekat" description="Jadwal kelas yang perlu Anda perhatikan berikutnya." action={<Link href="/siswa/kelas" className="text-theme-sm font-semibold text-limo-blue-700">Lihat kelas</Link>} />
        {dashboard.schedule.length > 0 ? <div className="grid gap-3 lg:grid-cols-2">{dashboard.schedule.map((item) => <article key={item.id} className="tailadmin-card p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-theme-xs font-semibold uppercase tracking-wide text-limo-blue-700">{item.kelas.name}</p><h2 className="mt-1 font-semibold text-gray-900">{item.topic}</h2><p className="mt-1 text-theme-sm text-gray-500">Pertemuan {item.meetingNumber} / {formatDateTime(item.sessionDate)}</p></div><span className="rounded-full bg-gray-50 px-3 py-1 text-theme-xs font-semibold text-gray-600">{formatUiLabel(item.status)}</span></div></article>)}</div> : <EmptyState icon="presensi" title="Belum ada jadwal terdekat" description="Jadwal akan tampil setelah kelas aktif Anda memiliki sesi pembelajaran." />}
      </section>

      {isFeatureEnabled("assignmentsEnabled") ? <section>
        <SectionHeader title="Tugas Online" description="Tugas yang perlu Anda kerjakan atau lanjutkan." action={<Link href="/siswa/kelas" className="text-theme-sm font-semibold text-limo-blue-700">Lihat kelas</Link>} />
        {dashboard.assignments.length > 0 ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{dashboard.assignments.map((item) => <Link key={item.id} href={`/siswa/tugas/${item.id}`} className="tailadmin-card block p-4 transition hover:border-limo-blue-200 hover:shadow-theme-sm"><p className="text-theme-xs font-semibold uppercase tracking-wide text-limo-blue-700">{item.kelas.name} / {formatUiLabel(item.submissionType)}</p><h2 className="mt-1 font-semibold text-gray-900">{item.title}</h2><p className="mt-2 text-theme-xs text-gray-500">{formatUiLabel(item.latestSubmission?.status, "Belum dimulai")}{item.dueAt ? ` / Tenggat ${formatDateTime(item.dueAt)}` : " / Tanpa tenggat"}</p></Link>)}</div> : <EmptyState icon="exam" title="Belum ada tugas online" description="Tugas dari Guru akan tampil di sini." />}
      </section> : null}

      <section className="grid gap-6 xl:grid-cols-2">
        <div>
          <SectionHeader title="Lanjutkan Belajar" description="Materi terbaru dari kelas yang Anda ikuti." />
          {dashboard.materials.length > 0 ? <div className="space-y-3">{dashboard.materials.map((item) => <Link key={item.id} href={`/siswa/kelas/${item.kelas.id}`} className="block tailadmin-card p-4 transition hover:border-limo-blue-200 hover:shadow-theme-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-theme-xs font-semibold uppercase tracking-wide text-limo-blue-700">{item.kelas.name}</p><LocalizedContent as="h2" text={item.title} language={item.language} direction="auto" className="mt-1 font-semibold text-gray-900">{item.title}</LocalizedContent></div><span className="rounded-full bg-limo-blue-50 px-2.5 py-1 text-[10px] font-semibold text-limo-blue-700">{formatUiLabel(item.type)}</span></div><p className="mt-2 text-theme-xs text-gray-500">Diperbarui {formatDate(item.updatedAt)}</p></Link>)}</div> : <EmptyState icon="materials" title="Belum ada materi" description="Materi dari Guru akan tampil di sini." />}
        </div>
