...
export default async function StudentGradebookPage({ params }: { params: Promise<{ kelasId: string }> }) {
  ...
  return <main className="space-y-6"><DashboardHero eyebrow={`${kelas.program.name} / ${kelas.level.name}`} title={`Nilai ${kelas.name}`} description="Nilai yang tampil sudah diterbitkan Guru. Draf buku nilai tidak terlihat di portal Siswa." actions={<Link href={`/siswa/kelas/${kelasId}`} className="tailadmin-button-outline px-4 py-2">Kembali ke Kelas</Link>} />{row ? <GradebookViewer row={row} weightTotal={data.weightTotal} /> : <EmptyState icon="exam" title="Belum ada data nilai" description="Nilai akan muncul setelah Guru menerbitkan komponen buku nilai." />}</main>;
}
