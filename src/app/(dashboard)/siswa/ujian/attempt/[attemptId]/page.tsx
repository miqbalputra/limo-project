...
export default async function StudentAttemptPage({ params }: { params: Promise<{ attemptId: string }> }) {
    ...
    return (
      <main className="space-y-6">
        <section className="tailadmin-card p-6 text-center">
          ...
          <p className="mt-2 text-theme-sm text-gray-500">Status saat ini: {formatUiLabel(attempt.status)}. Silakan kembali ke daftar ujian.</p>
          <Link href="/siswa/ujian" className="mt-4 tailadmin-button-primary px-4 py-2">Kembali ke Ujian</Link>
        </section>
      </main>
    );
  }
  ...
  return <main className="space-y-6"><OnlineExamPlayer attempt={normalizedAttempt} basePath="/api/v1/siswa" submittedHref="/siswa/ujian" /></main>;
}
