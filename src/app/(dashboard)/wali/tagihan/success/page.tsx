...
export default async function WaliPaymentSuccessPage({ searchParams }: { searchParams: Promise<{ tagihanId?: string; anak?: string }> }) {
  ...
  if (anak !== tagihan.siswa.id) {
    redirect(withWaliChildContext("/wali/tagihan/success", tagihan.siswa.id, new URLSearchParams({ tagihanId })));
  }
  ...
  return (
    <main className="space-y-6">
      <DashboardHero
        ...
        actions={<Link href={withWaliChildContext("/wali/tagihan", tagihan.siswa.id)} className="tailadmin-button-outline px-4 py-2">Kembali ke Tagihan</Link>}
        aside={<div className={`rounded-2xl p-5 shadow-theme-lg ${paid ? "bg-success-500 text-white" : "bg-warning-500 text-white"}`}><p className="text-theme-xs text-white/75">Status tagihan</p><p className="mt-1 text-xl font-semibold">{formatUiLabel(tagihan.status)}</p><p className="mt-1 text-theme-xs text-white/80">{tagihan.siswa.name}</p></div>}
      />
