  ...
  return (
    <main className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Link href={`/guru/sesi?kelasId=${kelasId}`} className="tailadmin-button-primary w-fit px-4 py-2">Kelola Sesi</Link><Link href={`/guru/materi?kelasId=${kelasId}`} className="tailadmin-button-outline w-fit px-4 py-2">Kelola Materi</Link>{isFeatureEnabled("learningModulesEnabled") ? <Link href={`/guru/kelas/${kelasId}/modul`} className="tailadmin-button-outline w-fit px-4 py-2">Susun Modul</Link> : null}{isFeatureEnabled("activityCompletionEnabled") && isFeatureEnabled("learningModulesEnabled") ? <Link href={`/guru/kelas/${kelasId}/progres`} className="tailadmin-button-outline w-fit px-4 py-2">Progres Aktivitas</Link> : null}{isFeatureEnabled("assignmentsEnabled") ? <Link href={`/guru/kelas/${kelasId}/tugas`} className="tailadmin-button-outline w-fit px-4 py-2">Kelola Tugas</Link> : null}{isFeatureEnabled("remedialEnabled") && isFeatureEnabled("assignmentsEnabled") ? <Link href={`/guru/kelas/${kelasId}/remedial`} className="tailadmin-button-outline w-fit px-4 py-2">Remedial</Link> : null}{isFeatureEnabled("gradebookEnabled") ? <Link href={`/guru/kelas/${kelasId}/gradebook`} className="tailadmin-button-outline w-fit px-4 py-2">Buka Buku Nilai</Link> : null}{isFeatureEnabled("classDiscussionEnabled") ? <><Link href={`/guru/kelas/${kelasId}/pengumuman`} className="tailadmin-button-outline w-fit px-4 py-2">Pengumuman</Link><Link href={`/guru/kelas/${kelasId}/diskusi`} className="tailadmin-button-outline w-fit px-4 py-2">Diskusi</Link></> : null}</div>
      </div>
