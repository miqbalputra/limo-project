...
export default async function GuruPresensiPage() {
  ...
  return (
    <main className="space-y-6">
      <div>
        <h1 className="tailadmin-page-title">Presensi</h1>
        <p className="mt-2 tailadmin-muted">Pilih sesi kelas untuk input presensi dan progres.</p>
      </div>
      <section className="grid gap-4 md:grid-cols-2">
        {sesiGroups.flatMap((group) => group.sesi.map((sesi) => (
          <article key={sesi.id} className="tailadmin-card p-5">
            ...
            <Link href={`/guru/presensi/${sesi.id}`} className="mt-4 tailadmin-button-primary px-4 py-2">Input</Link>
          </article>
        )))}
      </section>
    </main>
