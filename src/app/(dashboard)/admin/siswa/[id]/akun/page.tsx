...
export default async function StudentAccountPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="mx-auto max-w-2xl space-y-6"><div><Link href={`/admin/siswa/${item.id}`} className="text-theme-sm font-semibold text-limo-blue-700">Kembali ke detail siswa</Link><h1 className="mt-2 tailadmin-page-title">Akun Siswa: {item.name}</h1><p className="mt-1 tailadmin-muted">Kelola akses portal siswa tanpa mengubah data biodata atau keanggotaan kelas.</p></div><StudentAccountForm studentId={item.id} studentName={item.name} defaultIdentifier={item.nomorInduk} account={item.siswaAccount} /></main>;
}
