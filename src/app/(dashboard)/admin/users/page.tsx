...
export default async function UsersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="space-y-6">
      <div><p className="text-theme-sm font-medium text-gray-500">Administrasi Akun</p><h1 className="mt-1 text-2xl font-semibold text-gray-900">Pengguna</h1><p className="mt-2 tailadmin-muted">Kelola status akun dan cabut sesi aktif.</p></div>
       ...
       <section className="tailadmin-card overflow-hidden">
        ...
        {items.map((item) => (
          <article key={item.id} className="grid gap-3 px-5 py-4 md:grid-cols-[1.2fr_120px_120px_1fr] md:items-center md:gap-4">
             ...
             <div><span className={`inline-flex rounded-full px-3 py-1 text-theme-xs font-semibold ${getUiToneClass(item.status)}`}>{formatUiLabel(item.status)}</span><p className="mt-1 text-[10px] text-gray-400">{item._count.sessions} sesi</p></div>
